...
wget -O a.7z http://192.168.1.111:8080/api/Windows%207%207070.7z
7z x a.7z
rm a.7z
cd
clear
...
curl -o "setup"$setname".sh" https://raw.githubusercontent.com/AnBui2004/termux/refs/heads/main/setupwindows"$setname"two.sh
chmod +rwx "setup"$setname".sh"
echo qemu-system-x86_64 -usb -device usb-tablet -device usb-kbd -cpu qemu64,+sse,+sse2,+sse4.1,+sse4.2 -smp sockets=1,cores=4,threads=1 -m 2048M -overcommit mem-lock=off -drive file=/storage/emulated/0/VM/VMW77070.qcow2,aio=threads,cache=unsafe -device qxl-vga,vgamem_mb=128 -device intel-hda -device hda-duplex -device e1000,netdev=n0 -netdev user,id=n0 -accel tcg,thread=multi,tb-size=2048 -rtc base=2009-03-24t00:00:00 -vnc :2 > "start"$setname"vm.sh"
chmod +rwx "start"$setname"vm.sh"
cd ../
echo "/root/setup"$setname".sh" >> ./etc/profile
cd
echo 'sed -i '/start"$setname"/d' /data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/debian/etc/profile' > "start"$setname"vm.sh"
