...
wget -O a.7z https://archive.org/download/windows-7-6469.7znbabtermux/Windows%207%206469.7z
7z x a.7z
rm a.7z
cd
clear
...
echo qemu-system-x86_64 -usb -device usb-tablet -device usb-kbd -cpu qemu64,+sse,+sse2,+sse4.1,+sse4.2 -smp sockets=1,cores=4,threads=1 -m 2048M -overcommit mem-lock=off -drive file=/storage/emulated/0/VM/W76469.qcow2,aio=threads,cache=unsafe -device qxl-vga,vgamem_mb=128 -device ac97 -device e1000,netdev=n0 -netdev user,id=n0 -accel tcg,thread=multi,tb-size=2048 -rtc base=2007-10-02t00:00:00 -vnc :2 > "start"$setname"vm.sh"
