...
wget -O a.7z 'https://www.googleapis.com/drive/v3/files/1vzhnDBuFCZ5u4_Qo9nFUlJRdwzZtLE_i?alt=media&key=AIzaSyBg5cJ4QZF7kDgco9Sxrs-cPtV3F65bVSY'
7z x a.7z
rm a.7z
cd
clear
...
echo 'qemu-system-x86_64 -usb -device usb-tablet -device usb-kbd -cpu qemu64,+sse,+sse2,+sse4.1,+sse4.2 -smp sockets=1,cores=4,threads=1 -overcommit mem-lock=off -m 2048M -drive file=/storage/emulated/0/VM/W77000.qcow2,aio=threads,cache=unsafe -device qxl-vga,vgamem_mb=128 -device intel-hda -device hda-duplex -device e1000,netdev=n0 -netdev user,id=n0 -accel tcg,thread=multi,tb-size=2048 -rtc base=2008-12-13t00:00:00 -vnc :2' > "start"$setname"vm.sh"
